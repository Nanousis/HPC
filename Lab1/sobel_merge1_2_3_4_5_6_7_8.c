...
int convolution2D_horiz(const int posy, const int posx, const unsigned char *input);
int convolution2D_vert(const int posy, const int posx, const unsigned char *input);
...
int convolution2D_horiz(const int y, const int x, const unsigned char *in){
    int res = 0;

    // Row y-1: [-1, 0, 1]
    res += -1 * in[(y-1)*SIZE + (x-1)];
    res +=      in[(y-1)*SIZE + (x+1)];
...
int convolution2D_vert(const int y, const int x, const unsigned char *in){
    int res = 0;
...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	memset(&output[SIZE*(size_minus1)], 0, SIZE*sizeof(unsigned char));
	for (i = 1; i < size_minus1; i++) {
		output[i*SIZE] = 0;
		output[i*SIZE + size_minus1] = 0;
	}
	...
	for (i=1; i<size_minus1; i+=1 ) {
		for (j=1; j<(size_minus1); j+=1) {
			/* Apply the sobel filter and calculate the magnitude *
			 * of the derivative.								  */
			/* inline convolution2D_horiz */
			conv_res1 = 0;
			conv_res1 += -1 * input[size_ptr-SIZE + (j-1)];
			conv_res1 +=      input[size_ptr-size_minus1 + j];
			conv_res1 += -2 * input[size_ptr + (j-1)];
			conv_res1 +=  2 * input[size_ptr + (j+1)];
			conv_res1 += -1 * input[size_ptr + size_minus1 + j];
			conv_res1 +=      input[size_ptr + SIZE + (j+1)];

			/* inline convolution2D_vert */
			conv_res2 = 0;
			conv_res2 +=  1 * input[size_ptr-SIZE + (j-1)];
			conv_res2 +=  2 * input[size_ptr-SIZE + (j  )];
			conv_res2 +=  1 * input[size_ptr-size_minus1 + j];
			conv_res2 += -1 * input[size_ptr + size_minus1 + j];
			conv_res2 += -2 * input[size_ptr + SIZE + (j  )];
			conv_res2 += -1 * input[size_ptr + SIZE + (j+1)];
		...
		for ( j=4; j<size_minus1; j+=UNROLL_FACTOR ) {
